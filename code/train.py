...
parser.add_argument('--sample_time', type=int, default=1,
                    help='sample frequency, e.g. 2 means 1 sample every 2 seconds')
parser.add_argument('--padding', action='store_true', default=False,
                    help='whether input vehicle features padded. Padded parts will be masked for loss calculation')
parser.add_argument('--data_edge_attr', type=str, default=None,
                    help='edge attribute type to be loaded. Not used for final model')
parser.add_argument('--train_file_name', type=str, default='simulation_processed',
                    help='parent dir for training data file')
...
parser.add_argument('--encode_dim', type=int, default=-1,
                    help='dimension of final encode embedding vector. If -1, set to the same as hidden_dim')
...
parser.add_argument('--model_self_loop', action='store_true', default=False,
                    help='add self loop for GAT and GATv2 graph layers')
parser.add_argument('--model_edge_attr', action='store_true', default=False,
                    help='use pre-defined edge weights in the model')
parser.add_argument('--head_concat', action='store_true', default=False,
                    help='concatenate attention heads (and pass through a fc layer) instead of averaging')
parser.add_argument('--n_head', type=int, default=3, help='# attention head')
parser.add_argument('--no_decode_graph', action='store_true', default=False,
                    help='whether to use graph information at decoding')
parser.add_argument('--vq_embedding_dim', type=int, default=10, help="vq embedding dimension or feature dimension")
parser.add_argument('--vq_embedding_num', type=int, default=256, help="vq embedding num, usually 2**n")
parser.add_argument('--vq_commitment_cost', type=float, default=0.25, help='vq commitment cost')

# training param
parser.add_argument('--learning_rate', type=float, default=5e-2)
parser.add_argument('--lr_decay_step_size', type=int, default=50,
                    help='decrease lerarning rate by half every lr_decay_step_size')
parser.add_argument('--clip_grad', type=float, default=1)
parser.add_argument('--loss_rate_lane', type=float, default=2,
                    help='weighting paramter in loss function for the lane loss term')
parser.add_argument('--loss_rate_acc', type=float, default=2,
                    help='weighting paramter in loss function for the acceleration loss term')
parser.add_argument('--loss_rate_speed', type=float, default=1,
                    help='weighting paramter in loss function for the speed loss term')
parser.add_argument('--loss_rate_vq', type=float, default=2,
                    help='weighting parameter in loss function for the vq embedding loss term')
parser.add_argument('--GPU', type=str, default="1")
parser.add_argument('--epochs', type=int, default=100)
parser.add_argument('--experiment_name', type=str, default='vq_module_test5',
                    help='unique experiment name, used to save results to dir')
parser.add_argument('--verbal', action='store_true', default=False, help='print training results')
parser.add_argument('--batch_size', type=int, default=32)
parser.add_argument('--highD_data', action='store_true', default=False,
                    help='training using highD data. Otherwise use simulation data')
...
def train(epoch, train_loader, val_loader, best_val, best_epoch, verbal=False, padding=False):
    ...
    for i, graph_data in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        output, vq_loss = model(graph_data)  # [node, time, feature_dim]
        # print('train output shape: ', str(output.shape))
        ...
        if padding:
            ...
            graph_mask = []
        loss = cal_loss(output, vq_loss, graph_x, rate_lane, rate_speed, rate_acc, rate_vq, graph_mask, n_lane)
        print(f'epoch: {epoch}, batch: {i}, vq_loss: {vq_loss}, all_loss: {loss}')
        loss.backward()
    ...
    for graph_val in val_loader:
        ...
        with torch.no_grad():
            output, vq_loss = model(graph_val)  # [node, time, feature_dim]
        # print('eval output shape: ', str(output.shape))
...
for epoch in tqdm(range(0, n_epochs + 1)):
    train_loss, val_loss, best_val, best_epoch = train(epoch, train_loader, val_loader, best_val, best_epoch,
                                                       verbal=args.verbal, padding=args.padding)
