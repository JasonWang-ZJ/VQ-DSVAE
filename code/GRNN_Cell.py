...
class GRNN_Cell(torch.nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, K: int = 1, bias: bool = True, graph_layer='GATConv',
                 dropout=0, self_loop=True, head_concat=False, aggr='mean'):
        '''
    ...
    def _create_parameters_and_layers(self):
        ...
        if self.graph_layer == 'GATConv':
            self.conv_x_z = GATConv(in_channels=self.in_channels + self.out_channels, out_channels=self.out_channels,
                                    heads=self.K, concat=self.head_concat, dropout=self.dropout,
                                    add_self_loops=self.self_loop)

            self.conv_x_r = GATConv(in_channels=self.in_channels + self.out_channels,
                                    out_channels=self.out_channels, heads=self.K, concat=self.head_concat,
                                    dropout=self.dropout, add_self_loops=self.self_loop)
            ...
            self.conv_x_z = GATv2Conv(in_channels=self.in_channels + self.out_channels, out_channels=self.out_channels,
                                      heads=self.K, concat=self.head_concat, dropout=self.dropout,
                                      add_self_loops=self.self_loop)
            ...
            self.conv_x_z = TransformerConv(in_channels=self.in_channels + self.out_channels,
                                            out_channels=self.out_channels, heads=self.K, concat=self.head_concat,
                                            dropout=self.dropout)

            self.conv_x_r = TransformerConv(in_channels=self.in_channels + self.out_channels,
                                            out_channels=self.out_channels, heads=self.K, concat=self.head_concat,
                                            dropout=self.dropout)

            self.conv_x_h = TransformerConv(in_channels=self.in_channels + self.out_channels,
                                            out_channels=self.out_channels, concat=self.head_concat, heads=self.K,
                                            dropout=self.dropout)

        elif self.graph_layer == 'SAGEConv':
            self.conv_x_z = GraphConv(in_channels=self.in_channels + self.out_channels, out_channels=self.out_channels,
                                      aggr=self.aggr)
            self.conv_x_h = GraphConv(in_channels=self.in_channels + self.out_channels, out_channels=self.out_channels,
                                      aggr=self.aggr)
